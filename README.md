...
| Repo | Purpose |
|------|---------|
| **`nairatax-xyz`** _(this repo)_ | Org landing, this README, roadmap, project board |
| **`nairatax-web`** | Frontend app — dashboard, review/tag UI, reports, export |
| **`nairatax-engine`** | Ledger ingestion, event classification, FIFO cost basis |
| **`nairatax-rules`** | Jurisdiction rule packs as auditable data (e.g. `nigeria-nta-2025`) |
| **`nairatax-contracts`** | _(optional)_ Soroban contracts, if an on-chain component is added |
...
Not yet formally specified — as `nairatax-engine` and `nairatax-rules` take
shape, the event schema (`in` / `out` / `swap`) and the rule-pack format
become the cross-repo contracts to keep in sync. Document them here once
they're defined.
